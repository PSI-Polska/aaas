...
class GrpcEngine(val connectionProvider: ConnectionProvider<AAASGrpc.AAASBlockingStub>) : Engine<CalculationDefinition, Parameters, Parameters> {
    ...
    override fun call(calcDef: CalculationDefinition): Parameters? {
        ...
        try {
            ...
            return retParams.params.valueMap.map { it.key to fromCommParams(it.value.vector, calcDef.outParameters[it.key]) }.toMap()
        } catch (ex: Exception) {
    ...
    @Suppress("UNCHECKED_CAST")
    fun fromCommParams(vector: pl.psi.aaas.communication.Vector, parameter: Parameter<*>?): Vector<Any> =

            if (parameter != null && parameter is Vector<*>)
                ...
                else if (vector.hasEmptyValue()) parameter as Vector<Any> //this is an super uber fancy staff - that relies on calcdef.outparams empty arrays contracts
                    // TODO: because of of wildcard in Parameter<*>, we can't get value from value - we need to fix that
//                else if (vector.hasVectorValue()) {
//                    val array = vector.vectorValue.vectorList.map { fromCommParams(it, parameter.value as Parameter<*>?) }.toTypedArray()
...
//                }
    ...
    private fun getEmpty(clazz: Class<out Any>): Vector<Any> {
        clazz as Class<Any>
        return Parameter.ofArray(emptyArray<Any?>(), clazz)
    }
