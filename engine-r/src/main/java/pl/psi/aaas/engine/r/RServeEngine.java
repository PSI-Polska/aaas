...
public class RServeEngine< D extends CalculationDefinition, V, R > implements Engine< D, V, R >
{
    ...
    private static void sourceScript( RConnection conn, CalculationDefinitionIf calcDef, String scriptPath )
    ...
    public R call( D calcDef ) throws CalculationException
    {
        try
        {
            ...
            try
            {
                ...
                for( Map.Entry< String, Parameter< ? > > entry : calcDef.getInParameters()
                    .entrySet() )
                {
                    if( entry != null )
                    {
                        RValuesTransceiver< Parameter< ? >, ?, CalculationDefinitionIf > t =
                            RTransceiverFactory.get( entry.getValue(), conn );
                ...
                for( Map.Entry< String, Parameter< ? > > entry : calcDef.getOutParameters()
                    .entrySet() )
                {
                    if( entry != null )
                    {
        ...
        catch( RserveException ex )
        {
            ...
            throw new CalculationException(
                ex.getMessage() != null ? ex.getMessage() : "There was an error during calculation.", ex );
        }
        catch( Exception ex )
        {
