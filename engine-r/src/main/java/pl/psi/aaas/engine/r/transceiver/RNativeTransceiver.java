...
class RNativeTransceiver<V extends Parameter<?>, R> implements RValuesTransceiver<V, R, CalculationDefinitionIf > {
    private final RConnection session;
    private final Function<V, REXP> outTransformer;
    private final Function<REXP, R> inTransformer;

    public RNativeTransceiver(RConnection session, Function<V, REXP> outTransformer, Function<REXP, R> inTransformer) {
    ...
    @Override
    public R receive(String name, Object result, CalculationDefinitionIf definition) throws CalculationException {
        try {
            REXP rexpResult;
