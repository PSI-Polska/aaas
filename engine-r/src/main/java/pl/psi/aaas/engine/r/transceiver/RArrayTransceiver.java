...
class RArrayTransceiver< V extends Parameter< Object[] >, R > extends RNativeTransceiver< V, R >
{
    ...
    public RArrayTransceiver( RConnection session, Function< V, REXP > outTransformer,
        Function< REXP, R > inTransformer )
    {
        super( session, outTransformer, inTransformer );
    }

    public static RValuesTransceiver< Parameter< String[] >, String[], CalculationDefinitionIf > string(
        RConnection session )
    {
        return new RNativeTransceiver<>( session, v -> new REXPString( v.getValue() ), r -> {
            try
            {
    ...
    public static RValuesTransceiver< Parameter< Long[] >, Long[], CalculationDefinitionIf > longTransceiver(
        RConnection session )
    {
        return new RNativeTransceiver<>( session, v -> {
    ...
    public static RValuesTransceiver< Parameter< Double[] >, Double[], CalculationDefinitionIf > doubleTransceiver(
        RConnection session )
    {
        return new RNativeTransceiver<>( session, v -> {
            double[] doubles = Arrays.stream( v.getValue() )
    ...
    public static RValuesTransceiver< Parameter< Boolean[] >, Boolean[], CalculationDefinitionIf > booleanTransceiver(
        RConnection session )
    {
        return new RNativeTransceiver<>( session, v -> {
            ...
            for( int i = 0; i < v.getValue().length; i++ )
            {
                ...
                if( b == null )
                    bytes[ i ] = REXPLogical.NA;
                else if( b )
                    bytes[ i ] = REXPLogical.TRUE;
                else
                    bytes[ i ] = REXPLogical.FALSE;
            }
            ...
            try
            {
                if( r.isLogical() )
                {
                    ...
                    {
                        if( bytes[ i ] == REXPLogical.TRUE )
                            result[ i ] = true;
                        else if( bytes[ i ] == REXPLogical.FALSE )
                            result[ i ] = false;
                        else
                            result[ i ] = null;
                    }
