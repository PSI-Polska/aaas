...
class DataFrameTransceiver implements RValuesTransceiver< DataFrame, DataFrame, CalculationDefinitionIf >
{
    ...
    private List< Q > findAllTransceivers( DataFrame df ) throws CalculationException
    {
        List< Q > result = new ArrayList<>();
        for( Column col : df.getValue() )
        {
            RValuesTransceiver< Parameter< ? >, ?, CalculationDefinitionIf > transceiver =
                RTransceiverFactory.get( col.getVector(), session );
            result.add( new Q( col.getSymbol(), (Vector< ? >)col.getVector(),
                (RValuesTransceiver< Vector< ? >, ?, CalculationDefinitionIf >)(Object)transceiver,
                (Class< Object >)col.getVector()
                    .getElemClazz() ) );
        }
        return result;
    }
    ...
    @Override
    public void send( String name, DataFrame value, CalculationDefinitionIf definition )
    ...
    @Override
    public DataFrame receive( String name, Object result, CalculationDefinitionIf definition )
        throws CalculationException
    {
        try
        {
            ...
            Parameter< ? > param = definition.getOutParameters()
                .get( name );
    ...
    private static class Q
    {
        ...
        Q( String symbol, Vector< ? > vector,
            RValuesTransceiver< Vector< ? >, ?, CalculationDefinitionIf > transceiver, Class< Object > clazz )
        {
            this.symbol = symbol;
            this.vector = vector;
