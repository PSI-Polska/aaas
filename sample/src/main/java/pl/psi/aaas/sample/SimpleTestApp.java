...
public class SimpleTestApp
{
    ...
    private static CalculationDefinitionIf prepCalcDef1()
    {
        String[] strArr = { "a", "b", "c" };
        ZonedDateTime[] dtArr =
            { ZonedDateTime.now().minusHours( 2 ), ZonedDateTime.now().minusHours( 1 ), ZonedDateTime.now() };
        ...
        Vector< Boolean > boolVec =
            Parameter.ofArrayNotNull( new Boolean[]{ true, false, true }, Boolean.class );
        Vector< Boolean > boolNullVec =
            Parameter.ofArray( new Boolean[]{ true, false, null }, Boolean.class );

        Column[] dfColumns =
            { new Column( "dt", dtVec ), new Column( "longs", longVec ), new Column( "doubles", doubleVec ) };
    ...
    {
        @Override
        public pl.psi.aaas.engine.r.REngineConfiguration getConfiguration()
        {
            return new pl.psi.aaas.engine.r.REngineConfiguration( "192.168.99.100", 6311 );
        }
    }
    ...
    public static class FixedTSFacade implements Facade< CalculationDefinitionIf, Void >
    {
        ...
        private final Engine< CalculationDefinitionIf, Map< String, Parameter< ? > >, Map< String, Parameter< ? > > >
            engine = new RServeEngine( new LocalRConnectionProvider() );
